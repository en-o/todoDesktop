...
- **桌面框架**: Tauri 1.5
- **后端语言**: Rust
- **Git 操作**: git2-rs (Rust) + System Git
- **Markdown 编辑**: Vditor (即时渲染模式)
- **状态管理**: Zustand
- **构建工具**: Vite
...
**脚本功能：**
- 检查未提交的更改
- 自动更新 `package.json`、`tauri.conf.json`、`Cargo.toml` 中的版本号
- 创建 Git 标签
- 推送到远程仓库触发 GitHub Actions 自动构建
...
| 平台 | 文件 |
|------|------|
| Windows | `.msi` 安装包、`.exe` 安装程序、便携版 `.zip` |
...
**文件命名规则：**
- Todo 文件：`年/月/MM-DD.md`
- 附件文件：`年/月/assets/MM-DD-文件名.ext`
